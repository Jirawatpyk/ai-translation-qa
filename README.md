...
- **Cowork:** ตอนเพิ่ม marketplace ให้เปิด **Sync automatically** ไว้ รุ่นใหม่จะเข้ามาเองโดยไม่ต้องทำอะไร ถ้าปิดไว้ ให้กด Update ที่ marketplace หรือที่ plugin
...
1. **1 แชทต่อ 1 งาน ไม่ใช่ 1 แชทต่อลูกค้า**
   - งาน = การส่งมอบหนึ่งรอบ ถ้ารอบนั้นมีหลายไฟล์หรือหลายภาษา ให้อยู่ในแชทเดียวกัน เพราะขั้นตรวจความสม่ำเสมอต้องเห็นทุกไฟล์และทุกภาษาพร้อมกัน
   - ลูกค้าเดิมแต่คนละ PO หรือคนละรอบส่ง ให้เปิดแชทใหม่ ความรู้เรื่องลูกค้าไม่ได้หายไปไหน ศัพท์และคำแปลที่ส่งแล้วเก็บอยู่ใน TM ของ Project และงานใหม่จะดึงมาใช้เอง
2. **ตั้งชื่อแชทแบบ `ลูกค้า – เลขงาน – คู่ภาษา`** เช่น `Acme – PO1234 – en>th` เพราะทุกคนใน account เห็นรายการแชทชุดเดียวกัน
3. **บอกชื่อลูกค้าปลายทางตั้งแต่ข้อความแรก** ถ้าไม่บอก TM ของงานนั้นจะไม่ถูกบันทึก
4. **ลูกค้าตอบ query หรือขอแก้รอบสองของงานเดิม ให้ทำต่อในแชทเดิม** ถ้าแชทเดิมผ่านมานานจนไฟล์ทำงานหายไปแล้ว ให้เปิดแชทใหม่ แนบไฟล์ที่ส่งมอบไปแล้วกับ QA report แล้วบอกว่าเป็นงานต่อจากแชทไหน
5. **ลูกค้ายืนยันศัพท์หลังปิดงานไปแล้ว** ให้เปิดแชทสั้น ๆ ใน Project แล้วพิมพ์ เช่น "ลูกค้า Acme ยืนยันคำ checkout = ชำระเงิน" ระบบจะอัปเกรดคำนั้นใน termbase เป็น confirmed
6. **อย่าทำงานของลูกค้าและคู่ภาษาเดียวกันพร้อมกันสองแชท** ถ้าสองแชทเขียน TM ไฟล์เดียวกันพร้อมกัน ข้อมูลบางส่วนอาจหาย
